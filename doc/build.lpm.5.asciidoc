...
Name
----
build.lpm - Package build description file
...
Description
-----------
This manual page describes general rules about build.lpms. Once a
build.lpm is written, the actual package is built using makepkg and installed
with lpm.

NOTE: An example build.lpm, useful for reference, is located in '{pkgdatadir}'
along with other example files such as an install script. You can copy the
provided build.lpm.proto file to a new package build directory and make
...
in a build.lpm. These are all understood and interpreted by makepkg, and most
of them will be directly transferred to the built package. The mandatory
fields for a minimally functional build.lpm are *pkgname*, *pkgver*, *pkgrel*
and *arch*.
...
function in the build.lpm that outputs the new package version.
...
*pkgrel*::
	...
	software release and incremented for intermediate build.lpm updates. The
...
*epoch*::
	...
	version comparison logic. See linkman:lpm[8] for more information on
	version comparisons.
...
*install*::
	...
	This file should reside in the same directory as the build.lpm and will
...
*source (array)*::
	...
	must either reside in the same directory as the build.lpm, or be a
	fully-qualified URL that makepkg can use to download the file.
	To simplify the maintenance of build.lpms, use the `$pkgname` and `$pkgver`
...
*cksums (array)*::
	...
	be skipped. To easily generate cksums, run ``makepkg -g >> build.lpm''.
...
*backup (array)*::
	...
	`"Handling Config Files"` in linkman:lpm[8] for more information.
...
*optdepends (array)*::
	...
	and are not utilized by lpm during dependency resolution. Packages in
...
+
Sysupgrade is currently the only lpm operation that utilizes this field.
...
Packaging Functions
-------------------
In addition to the above directives, build.lpms require a set of functions that
provide instructions to build and install the package. As a minimum, the
build.lpm must contain a `package()` function which installs all the package's
...
*startdir*::
	This contains the absolute path to the directory where the build.lpm is
...
Package Splitting
-----------------
makepkg supports building multiple packages from a single build.lpm. This is
...
given in the build.lpm. Nevertheless, the following ones can be overridden within
...
--------------------------------
Lpm has the ability to store and execute a package-specific script when it
...
same directory as the build.lpm script. Then use the install directive:
...
Example
-------
The following is an example build.lpm for the 'patch' package. For more
examples, look through the build files of your distribution's packages.

[source,sh]
-------------------------------
include::build.lpm-example.txt[]
-------------------------------


See Also
--------
linkman:makepkg[8], linkman:lpm[8], linkman:makepkg.conf[5]
