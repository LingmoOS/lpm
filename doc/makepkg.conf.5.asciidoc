...
Synopsis
--------
{sysconfdir}/makepkg.conf, $XDG_CONFIG_HOME/lpm/makepkg.conf, ~/.makepkg.conf
...
$XDG_CONFIG_HOME/lpm/makepkg.conf or ~/.makepkg.conf, with the former
taking priority.
...
**DLAGENTS=(**\'protocol::/path/to/command [options]' ...**)**::
	...
	the linkman:build.lpm[5] file. Options can be specified for each command as
...
**VCSCLIENTS=(**\'protocol::package' ...**)**::
	...
	in the `depends` or `makedepends` arrays in the build.lpm.
...
**BUILDENV=(**!distcc !color !ccache check !sign**)**::
	...
	*ccache*;;
		...
		build.lpm options array.

	*check*;;
		Run the check() function if present in the build.lpm. This can be
...
**BUILDDIR=**"/path/to/directory"::
	...
	Incorrect use of `$startdir` in a build.lpm may cause building with
	this option to fail.
...
**OPTIONS=(**!strip docs libtool staticlibs emptydirs ...**)**::
	...
	equivalent to options that can be placed in the build.lpm; the defaults are
...
**PKGDEST=**"/path/to/directory"::
	...
	current directory (location of the linkman:build.lpm[5]). Many people
...
**LPM_AUTH=()**::
	Specify a command prefix for running lpm as root. If unset, makepkg will
...
See Also
--------
linkman:makepkg[8], linkman:lpm[8], linkman:build.lpm[5]
