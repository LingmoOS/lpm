...
Name
----
lpm - package manager utility


Synopsis
--------
'lpm' <operation> [options] [targets]


Description
-----------
Lpm is a package management utility that tracks installed packages on a Linux
...
automatically upgrade packages. Lpm packages are a zipped tar format.

Since version 3.0.0, lpm has been the front-end to linkman:libalpm[3], the
``Lingmo OS Package Management'' library. This library allows alternative
front-ends to be written (for instance, a GUI front-end).

Invoking lpm involves specifying an operation with any potential options and
...
*-D, \--database*::
	...
	certain attributes of the installed packages in lpm's database. It
...
*-S, \--sync*::
	...
	example, `lpm -S qt` will download and install qt and all the
	...
	package to install: `lpm -S testing/qt`. You can also specify version
	requirements: `lpm -S "bash>=3.2"`. Quotes are needed, otherwise the
	shell interprets ">" as redirection to a file.
+
In addition to packages, groups can be specified as well. For example, if
gnome is a defined package group, then `lpm -S gnome` will provide a
...
+
Packages that provide other packages are also handled. For example, `lpm -S
...
+
You can also use `lpm -Su` to upgrade all packages that are out-of-date. See
<<SO,Sync Options>> below. When upgrading, lpm performs version comparison
...
*-T, \--deptest*::
	...
	This operation accepts no other options. Example usage: `lpm -T qt
	"bash>=3.2"`.
...
*-U, \--upgrade*::
	...
	on how lpm takes care of configuration files.
...
*-b, \--dbpath* <path>::
	...
	+{localstatedir}/lib/lpm+). This should not be used unless you know what
	you are doing.
...
*-r, \--root* <path>::
	...
	*NOTE*: If database path or log file are not specified on either the
	command line or in linkman:lpm.conf[5], their default location will
...
*\--cachedir* <dir>::
	...
	+{localstatedir}/cache/lpm/pkg+). Multiple cache directories can be
	specified, and they are tried in the order they are passed to lpm.
...
*\--gpgdir* <dir>::
	...
	(the default is +{sysconfdir}/lpm/gnupg+). This directory should contain
...
*\--hookdir* <dir>::
	...
	+{sysconfdir}/lpm/hooks+). Multiple hook directories can be specified
...
*\--noconfirm*::
	...
	this unless you want to run lpm from a script.
...
*-d, \--nodeps*::
	...
	lpm will always check a package's dependency fields to ensure that all
...
*\--noprogressbar*::
	...
	for scripts that call lpm and capture the output.
...
*\--ignore* <package>::
	Directs lpm to ignore upgrades of package even if there is one
	available. Multiple packages can be specified by separating them
	with a comma.

*\--ignoregroup* <group>::
	Directs lpm to ignore upgrades of all packages in 'group', even if
...
*-q, \--quiet*::
	...
	lpm's output is processed in a script. Search will only show package
...
*-n, \--nosave*::
	Instructs lpm to ignore file backup designations. Normally, when a
...
*-c, \--clean*::
	...
	currently unused sync databases to free up disk space. When lpm
	...
	linkman:lpm.conf[5]. Use one '\--clean' switch to only remove
...
*-u, \--sysupgrade*::
	...
	necessary.
+
Pass this option twice to enable package downgrades; in this case, lpm will
...
*-y, \--refresh*::
	...
	server(s) defined in linkman:lpm.conf[5]. This should typically be used
...
*-q, \--quiet*::
	...
	lpm's output is processed in a script, however, you may want to use
...
----------------------------
Lpm uses the same logic as 'rpm' to determine action against files that are
...
lpm -U /home/user/ceofhack-0.6-1-x86_64.pkg.tar.gz::
...
Configuration
-------------
See linkman:lpm.conf[5] for more details on configuring lpm using the
'lpm.conf' file.
