...
Name
----
lpm.conf - lpm package manager configuration file
...
Description
-----------
Lpm, using linkman:libalpm[3], will attempt to read lpm.conf each time it
is invoked. This configuration file is divided into sections or repositories.
Each section defines a package repository that lpm can use when searching
...
*RootDir =* /path/to/root/dir::
	Set the default root directory for lpm to install to. This option is
...
*DBPath =* /path/to/db/dir::
	...
	default is +{localstatedir}/lib/lpm/+. Most users will not need to set
	this option. *NOTE*: if specified, this is an absolute path and the root
	path is not automatically prepended.

*CacheDir =* /path/to/cache/dir::
	Overrides the default location of the package cache directory. The
	default is +{localstatedir}/cache/lpm/pkg/+. Multiple cache directories can be
...
*HookDir =* /path/to/hook/dir::
	...
	+{sysconfdir}/lpm/hooks+.  Multiple directories can be specified with
...
*GPGDir =* /path/to/gpg/dir::
	...
	files for GnuPG. The default is +{sysconfdir}/lpm/gnupg/+.
...
*LogFile =* /path/to/log/file::
	Overrides the default location of the lpm log file. The default
	is +{localstatedir}/log/lpm.log+. This is an absolute path and the root directory
	is not prepended.

*HoldPkg =* package ...::
	If a user tries to '\--remove' a package that's listed in `HoldPkg`,
	lpm will ask for confirmation before proceeding. Shell-style glob
	patterns are allowed.

*IgnorePkg =* package ...::
	Instructs lpm to ignore any upgrades for this package when performing
	a '\--sysupgrade'. Shell-style glob patterns are allowed.

*IgnoreGroup =* group ...::
	Instructs lpm to ignore any upgrades for all packages in this
...
*Architecture =* auto &| i686 &| x86_64 | ...::
	If set, lpm will only allow installation of packages with the given
...
*Color*::
	Automatically enable colors only when lpm's output is on a tty.
...
*DownloadUser =* username::
	...
	option is not set then the downloads are done as the user running lpm.
...
# next use servers as defined in the pkg_sources.list below
Include = {sysconfdir}/lpm/pkg_sources.list
--------
...
+
During parsing, lpm will define the `$repo` variable to the name of the
current section. This is often utilized in files specified using the 'Include'
directive so all repositories can use the same mirrorfile. lpm also defines
...
directory with these packages so lpm can find it when run with '\--refresh'.
...
in the configuration file as shown in the config example above. Lpm will now
use your package repository. If you add new packages to the repository,
remember to re-generate the database and use lpm's '\--refresh' option.
...
See Also
--------
linkman:lpm[8], linkman:libalpm[3]
