...
/** Output lpm version and copyright.
 */
static void version(void)
{
	...
	printf("Thanks for Pacman project and Arch Linux Team.\n");
	printf("\n");
}
...
static void setuseragent(void)
{
	...
	uname(&un);
	len = snprintf(agent, 100, "lpm/%s (%s %s) libalpm/%s",
...
int main(int argc, char *argv[])
{
	...
	pm_printf(ALPM_LOG_DEBUG, "lpm v%s - libalpm v%s\n", PACKAGE_VERSION, alpm_version());
	...
	switch(config->op) {
		...
		case PM_OP_REMOVE:
			...
			break;
		case PM_OP_UPGRADE:
			ret = lpm_upgrade(pm_targets);
			break;
		case PM_OP_QUERY:
			ret = lpm_query(pm_targets);
			break;
		case PM_OP_SYNC:
