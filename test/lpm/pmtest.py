...
class pmtest(object):
    ...
    def generate(self, lpm):
        ...
        ldconfig = os.path.basename(lpm["ldconfig"])
        ldconfigdir = os.path.join(self.root, os.path.dirname(lpm["ldconfig"][1:]))
        shell = lpm["scriptlet-shell"][1:]
    ...
    def run(self, lpm):
        if os.path.isfile(util.PM_LOCK):
            tap.bail("\tERROR: another lpm session is on-going -- skipping")
            return

        tap.diag("==> Running test")
        vprint("\tlpm %s" % self.args)
        ...
        prog = lpm["bin"]
        if not prog:
            prog = util.which(self.cmd[0], lpm["bindir"])
        ...
        if lpm["manual-confirm"]:
            cmd.append("--confirm")
        if lpm["debug"]:
            cmd.append("--debug=%s" % lpm["debug"])
        cmd.extend(shlex.split(self.args))

        if not (lpm["gdb"] or lpm["nolog"]):
        ...
        # Change to the tmp dir before running lpm, so that local package
